...
class YateIVR(YateAsync):
    ...
    async def ring_extension(self, extension):
        """
        Ring an extension on the same PBX
        :param extension: The extension to ring
        :return: True if the operation was successful, false otherwise
        """
        msg_params = {
            'message': "call.execute",
            'id': self.call_id,
            'callto': 'sip/'+extension.split(';')[0],
            'osip_Alert-Info': '<http://codeskulptor-demos.commondatastorage.googleapis.com/GalaxyInvaders/bonus.wav>;info=alert-event;x-line-id=0',
        }
    ...
    async def fork_call(self, extensions, callername):
        msg_params = {
            ...
            'maxcall': '300000',
            'osip_Alert-Info': '<http://codeskulptor-demos.commondatastorage.googleapis.com/GalaxyInvaders/bonus.wav>;info=alert-event;x-line-id=0',
            'callto.1': 'wave/play/./ivr/audio/waiting/crab.sln',
